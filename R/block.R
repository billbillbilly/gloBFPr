...
#' @param min_block_area numeric. Minimum block area in m² below which polygons
...
generate_block <- function(x,
  ...
  if (!is.null(enc_blocks) && any(covered_mask)) {
    enc_id_to_block         <- setNames(enc_blocks$block_id, enc_with_bldg)
  ...
  if (!is.null(raster_blocks_sf) && length(uncovered_idx) > 0) {
    patch_to_block           <- setNames(raster_blocks_sf$block_id,
