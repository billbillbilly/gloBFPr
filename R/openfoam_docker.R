...
make_block_mesh_dict <- function(domain, nx, ny, nz) {
  ...
  paste0(
    ...
    #   inlet  (0 4 7 3): (0,0,+z)x(0,+y,0) = (-yz,0,0) → -x ✓
    #   outlet (1 2 6 5): (0,+y,0)x(0,0,+z) = (+yz,0,0) → +x ✓
    #   ground (3 2 1 0): (+x,0,0)x(0,-y,0) = (0,0,-xy)  → -z ✓
    #   top    (4 5 6 7): (+x,0,0)x(0,+y,0) = (0,0,+xy)  → +z ✓
    #   y-min  (0 1 5 4): (+x,0,0)x(0,0,+z) = (0,-xz,0)  → -y ✓
    #   y-max  (3 7 6 2): (0,0,+z)x(+x,0,0) = (0,+xz,0)  → +y ✓
    "boundary\n(\n",
    "    inlet\n    {\n        type patch;\n        faces ((0 4 7 3));\n    }\n",
    "    outlet\n    {\n        type patch;\n        faces ((1 2 6 5));\n    }\n",
    "    ground\n    {\n        type wall;\n        faces ((3 2 1 0));\n    }\n",
...
make_fv_solution <- function() {
  paste0(
    ...
    "        \"(k|epsilon)\"   1e-4;\n",
    "    }\n}\n\n",
    # Conservative relaxation factors — prevents SIGFPE divergence on first
    # iteration when building wakes create steep gradients.
    # U/k/epsilon at 0.5 (was 0.7); p at 0.3 unchanged.
    "relaxationFactors\n{\n",
    "    fields\n    {\n        p    0.3;\n    }\n",
    "    equations\n    {\n",
    "        U        0.5;\n",
...
#'   \code{building_refinement} additional levels of 2× refinement.
...
prepare_openfoam_case <- function(
  ...
  # safe for Docker at 4 GB RAM (peak usage during flood-fill ≈ 2× cell count).
...
#' the \code{Allrun} script produced by \code{prepare_openfoam_case()}.
#' The script chains \code{blockMesh} → \code{snappyHexMesh} →
#' \code{simpleFoam} and writes per-solver log files in \code{case_dir}.
#'
...
#'   \code{reconstructPar} reassembles.  Typical speed-up is 3–6× on a
...
#'   \code{wait = TRUE}).
#'
#' @seealso \code{\link{prepare_openfoam_case}}
#'
#' @export
run_openfoam_docker <- function(
  ...
  if (!file.exists(allrun))
    stop("No Allrun script found in:\n  ", case_dir,
         "\nRun prepare_openfoam_case() first.", call. = FALSE)
  ...
  if (.Platform$OS.type == "unix" && grepl("^/var/folders", case_dir))
    stop(
      ...
      "  prepare_openfoam_case(case_dir = file.path(path.expand('~'), 'openfoam_demo'), ...)",
      call. = FALSE
    )
...
#'
#' Works for both the wind case (\code{simpleFoam}) and the nocturnal case
#' (\code{buoyantBoussinesqSimpleFoam}).  For the nocturnal case the slice
#' is already written during the solver run via the \code{pedestrianSlice}
#' function object; use \code{\link{read_foam_pedestrian_slice}} instead.
#'
...
#'   \code{U_mag} (speed), \code{Ux}, \code{Uy}.
#'
...
sample_foam_slice <- function(
  ...
  if (exit != 0) {
    log <- tryCatch(
      tail(readLines(file.path(case_dir, "log.postProcess")), 20),
  ...
  if (!is.null(buildings)) {
    if (!requireNamespace("sf", quietly = TRUE)) {
      ...
      p <- p + ggplot2::geom_polygon(
        ...
        fill        = "grey40",   # darker than na_colour so buildings stand out
        colour      = "grey20",
        linewidth   = 0.2,
        inherit.aes = FALSE
      )
...
add_flow_vectors <- function(
  ...
  mag_max <- max(sqrt(df$Ux^2 + df$Uy^2), na.rm = TRUE)
  if (mag_max == 0) return(p)
  fac <- spacing * scale * 0.8 / mag_max
