...
#' Building Green View Index (BGVI) for each floor level based on estimated number of floors.
#' Default is `FALSE`.
#' @param key character. (only required for `get_green_acc`) API key of OpenTopography.
#'
#' @return
#' `get_green_acc` returns an `sf` object identical to input \code{x},
#' with additional columns for:
#' \itemize{
...
#' @examples
#' \donttest{
#' result <- gloBFPr::get_green_acc(globfp_example[c(1:3),],
...
get_green_acc <- function(x = NULL,
  ...
  # filter greenspace based on area
