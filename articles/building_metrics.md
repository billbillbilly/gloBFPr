...
the same `sf` object with additional metric columns, so the usual
...
computes geometric and shape metrics such as footprint area, perimeter,
...
You can also request only a subset of metrics when you do not need the
full set.
...
estimates the number of neighboring buildings and centroid distance
...
Use a smaller radius for dense local context or a larger radius for
...
computes the distance from each building centroid to the nearest
qualifying green patch within a buffer. The source can be `metachm`,
`esri`, or `sentinel2`.

By default the distance is measured in a straight line. Every result
also carries a `dng_method` column recording how the value was obtained,
...
simple fact that people walk on streets. Set `network = "osm"` to
measure along real road and path centre lines instead.
[`get_dng()`](https://billbillbilly.github.io/gloBFPr/reference/get_metrics.md)
downloads a walkable OpenStreetMap network for the study extent
(motorways, trunk roads, and ways tagged `foot=no` are excluded), builds
a routing graph, and reports
...
The network is downloaded once for the whole study area rather than once
per building, so routing adds a single Overpass request regardless of
how many buildings you pass in. If Overpass is unreachable or returns
nothing,
[`get_dng()`](https://billbillbilly.github.io/gloBFPr/reference/get_metrics.md)
warns and falls back to straight-line distance rather than failing.
...
`dng_method` tells you which measure each row actually used. A building
falls back to `"euclidean"` when no path connects it to any green patch
— typically an isolated network fragment, or a building whose buffer
extends past the edge of the downloaded network. Check it before
interpreting results, and compare the two measures to see where the
...
compared pixel by pixel, so a higher `zoom` gives a finer target set at
the cost of runtime. And buildings connect to the graph at its vertices,
so the network is densified to about 20 m spacing before routing —
accurate enough for walking distances without inflating the graph.

### 4 Building Green View Index

[`get_bgvi()`](https://billbillbilly.github.io/gloBFPr/reference/get_metrics.md)
estimates Building Green View Index from building viewpoints. The DSM
can include metaCHM canopy height, while the visible-green feature layer
can come from height-filtered canopy, 2D map-tile greenspace, or the
union of both. An OpenTopography API key is required for the DEM. This
...
To compute directional BGVI, provide one or more directions and a field
of view in degrees. The function reuses each computed viewshed and
...
when you want to inspect the viewshed behind one building’s BGVI value.
It uses the same DSM and green-feature preparation as
[`get_bgvi()`](https://billbillbilly.github.io/gloBFPr/reference/get_metrics.md),
but computes a single viewpoint and returns the diagnostic rasters
...
You can also inspect a specific floor or supply an observer height
directly.
...
To compute GVI by floor, set `floor = TRUE`. Use `floor_step` to sample
every `n` floors and reduce runtime. The top estimated floor is always
included.
...
`quiet = TRUE` for batch workflows, or set it to `FALSE` when you want
